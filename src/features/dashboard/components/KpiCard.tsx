  ...
  return (
    <Card className="p-0">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 px-5 py-4">
        <CardDescription className="text-xs font-semibold uppercase tracking-wide text-surface-500 dark:text-slate-400">
          {title}
        </CardDescription>
        {icon ? <div className="text-surface-400 dark:text-slate-400">{icon}</div> : null}
      </CardHeader>
      <CardContent className="space-y-3 px-5 pb-5">
        <CardTitle className="text-2xl font-semibold">{value}</CardTitle>
        {helper ? <p className="text-sm text-surface-500 dark:text-slate-400">{helper}</p> : null}
        {trendLabel ? (
